...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    for i in input_string:
        if i.isdigit():
            digit_list.append(i)
    digit_alpha = ['zero','one','two','three','four','five','six','seven','eight','nine']
    digit_string = []
    for i in digit_list:
        digit_string.append(digit_alpha[int(i)])
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
        ...
        Examples:
            ...
            "alreadyCamel"
    """
    return ''.join([x.lower().title() for x in underscore_str.split('_')])
